...
# This is KRAKEN DERIVATIVES US's rate ($0.15/contract/side on 0.01 BTC / 0.5 ETH
# / 5 SOL contracts). Its venue is Bitnomial, so it belongs on the bitnomial rows
# ONLY. R479 applied it to the Coinbase rows too, because Coinbase's own fee was
# secondary-sourced at the time and there was nothing better to use.
R478_FEE_RT = {"BTC": 0.0463, "ETH": 0.0314, "SOL": 0.0811}

# R482 CORRECTION, queue item 7. Coinbase Derivatives' fee is now PRIMARY-sourced:
# $0.10 per contract per side, electronic, standing (no promotional component),
# effective trade date 2025-12-15, from CFTC submission #2025-75 (filed 2025-11-26),
# corroborated independently by Lincoln Park Financial's BIP contract page.
# Converted to % of price on Coinbase's OWN live contract sizes (0.01 BTC / 0.1 ETH
# / 5 SOL) - the 0.1 ETH contract is a FIFTH of Kraken's, which is why ETH goes from
# the cheapest coin under the stand-in to the most expensive under the real rate.
# Generated by step482_coinbase_fee_source.py; see step482_cde_fee_table.json.
R482_FEE_RT_CDE = {"BTC": 0.0276, "ETH": 0.0859, "SOL": 0.0460}


def fee_rt(venue, coin):
    """Round-trip EXCHANGE fee as % of price, for the venue that actually bills it.

    EXCHANGE FEE ONLY. On the retail path Coinbase Financial Markets adds its own
    commission, and CFM bills a PERCENTAGE of notional ("as low as 0.02%", a volume
    -tier floor). That component is UNSOURCED (coinbase.com 403s unauthenticated)
    and is deliberately not given a value here. See R482 section 3.
    """
    table = R482_FEE_RT_CDE if venue.startswith("coinbase") else R478_FEE_RT
    return table.get(coin, float("nan"))
...
def report():
    ...
    print("  R482 UPDATE: each venue now carries ITS OWN sourced fee. Coinbase = $0.10/side")
    print("  (CFTC filing #2025-75), Bitnomial/Kraken US = R478's $0.15/side. The Kraken")
    print("  stand-in on the Coinbase rows is gone. These are EXCHANGE fees only - the")
    print("  retail FCM commission (a percentage, 'as low as 0.02%') sits on top, unsourced.")
    ...
    print("  and after R482, with each venue's own fee rather than one venue's on both.")
