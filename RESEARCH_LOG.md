...
# R494 — BACKFILL THE TAPE THAT THE SCREEN SAYS IS MISSING

**2026-09-06. Queue item 18. `step494_backfill_screen_tape.py`, full output in
`step494_output.txt`; the pre-backfill run of the same screen, captured before
a byte of new tape existed, in `step494_screen_before_backfill.txt`. Research
only, no orders, no account endpoints, no live file touched, imported or
edited. PLUMBING, NOT A HYPOTHESIS. NO LOOK CONSUMED, and none could be —
`simulate()` is never called anywhere in the file, no entry population is
scored, and every measurement stops at the same 80% boundary R489 fixed.**

## What this round was for

Item 18, opened by R489. R489's screen was limited by DATA, not by ideas, in
four places: **AVAXUSD and DOGEUSD had about four days of 1-minute tape each**
and were reported as data-absent rather than ranked; **ADAUSD, DOTUSD and
PAXGUSD were ranked off 118-132 days.** Alpaca had already served this repo
5.5 years of BTC/ETH/SOL/LINK/LTC 1-minute bars, so the history very likely
existed for the rest. Download it, write it in the existing cache convention,
and re-run the screen unchanged to see whether the ranking moves.

## The fence, fixed in the file's docstring before the first request went out

Six clauses, and two of them did real work.
1. **The five symbols are hard-coded and all INTACT.** ADA/DOT/PAXG are on
   R489's explicit intact list; AVAX and DOGE have never had an entry
   population built on them in any round in this log. No spent instrument is
   fetched, rewritten or re-windowed — BTC, ETH, SOL, LINK, LTC, SPY and QQQ
   are not touched.
2. Nothing is re-sliced. **Backfilling tape does not unspend a slice**, and
   nothing was ever spent on these five, so the question does not arise.
3. **Every file is capped at the corpus boundary, 2026-07-26.** Every other
   cache in this repo ends there and every window in R485-R493 is cut against
   it. Letting five files run six weeks further would silently have made the
   screen a comparison of different windows. **Backfill means BACKWARD.**
4. **The screen is re-run UNCHANGED** — executed as a subprocess byte for byte
   as it stands, mtime verified unchanged in the output. Its own fences hold
   unmodified on the new tape.
5. Atomic writes; every superseded parquet copied to `data_pre494/` rather
   than deleted, so R489's published numbers stay reproducible off the exact
   bytes they were computed from.
6. **A shorter frame is never written over a longer one.** A backfill that
   loses history is a bug, not a backfill.

## What the source actually served

| symbol | tf | was (days) | now (days) | gained | bars now | 1m coverage | gaps |
|---|---|---|---|---|---|---|---|
| ADAUSD | 1m | 163 | 163 | **+0** | 168,115 | 71.4% | 37,270 |
| ADAUSD | 5m | 163 | 163 | +0 | 46,069 | 97.8% | 872 |
| DOTUSD | 1m | 147 | **1073** | **+926** | 1,256,982 | 81.3% | 177,487 |
| DOTUSD | 5m | 1073 | 1073 | +0 | 305,828 | 98.9% | 2,773 |
| PAXGUSD | 1m | 147 | **2033** | **+1886** | 311,292 | **10.6%** | 115,538 |
| PAXGUSD | 5m | 163 | **2033** | **+1869** | 129,803 | 22.2% | 42,151 |
| AVAXUSD | 1m | 3 | **1711** | **+1708** | 1,980,264 | 80.4% | 261,570 |
| AVAXUSD | 5m | 8 | 1711 | +1703 | 482,218 | 97.8% | 8,261 |
| DOGEUSD | 1m | 3 | **2033** | **+2030** | 2,373,535 | 81.1% | 296,172 |
| DOGEUSD | 5m | 8 | 2033 | +2025 | 573,175 | 97.9% | 9,478 |

Nine of ten files grew; **AVAX went from 3 days to 1,711 and DOGE from 3 to
2,033.** Every write passed the integrity gate (no duplicate stamps, no
out-of-order rows, no bar whose high is below its own open or close, no
non-positive price).

**ADA is the one that did not move, and it is a fact about the VENDOR, not the
coin.** Alpaca serves no ADA/USD bar of any resolution before 2026-02-13; a
probe from 2015-01-01 returns that same first bar. ADA stays a 132-day
instrument and is reported as one.

**PAXG's 1-minute tape is genuinely sparse: 10.6% coverage, about 153 bars a
day out of 1,440.** Gold on a crypto venue prints when it trades. The screen's
volatility is gap-clean by construction (only consecutive bars exactly 60s
apart contribute) so this does not corrupt the coordinate — but it is why
PAXG is now the one 24/7 crypto row where the gap-clean and raw columns
**disagree** (0.0791% against 0.1103%), and the screen's own prose says that
divergence is supposed to be a session-instrument signature. Read PAXG's rows
knowing the tape underneath them is a tenth full.

## (1) THE RANKING MOVED. THE TOP TWO DID NOT.

R489's answer to item 13 — **XRPUSD and LINKUSD** — survives 8x to 700x more
tape on four instruments. XRP is still first on the snapshot read, LINK still
first on the fee-only read, exactly as published.

    intact-slice order BEFORE: XRP > LINK > LTC > PAXG > ADA > DOT
    intact-slice order AFTER : XRP > LINK > DOGE > PAXG > LTC > ADA > DOT > AVAX
    fee-only order BEFORE    : LINK > XRP > PAXG > LTC > ADA > DOT
    fee-only order AFTER     : LINK > PAXG > XRP > DOGE > LTC > ADA > DOT > AVAX

Three things changed underneath that stable head.

**DOGEUSD arrives as a real candidate: 1,627 days, INTACT, third among intact
instruments on the snapshot read (multiple 0.65) and fifth on fee-only
(1.51).** It was "DATA ABSENT — not a candidate" nine hours ago. Its
volatility reads 0.1007% of price a minute, second only to SOL among
everything on this disk.

**PAXGUSD's coordinate was wrong by 69% and it moves gold up the table.** On
118 days its 1-minute move read 0.0467%; on 850 it reads **0.0791%**. Because
its $4,433 contract sits far above R493's $750 break, PAXG pays the flat
0.04% floor, so on the fee-only read it goes from **1.17 (5th) to 1.98 (2nd),
ahead of XRP.** R493 handed the gold specialist a sourced cost six days ago
and concluded the R488 multiple was **0.58 — "gold's median minute does not
clear its own round trip."** On 7x the tape that figure is **1.98 fee-only /
0.63 all-in.** R493's sentence was true of 118 days of gold and is false of
850.

**AVAXUSD arrives and ranks LAST, for exactly the reason R493 predicted.** Its
tape is fine — 0.0973% a minute, fourth-highest on the disk — but **AVAX PERP
is a $76.60 contract**, deep under the $750 break, so the $0.15 minimum
charges it **0.3916% a round trip** and the multiple collapses to 0.25.
Nothing about the instrument disqualifies it; the contract size does. That is
R493's rule arriving as a live ranking, on an instrument R493 never saw.

DOT gained 926 days and barely moved: volatility 0.0824% → 0.0842%, and it
stays at the bottom of the CDE band on its $95.30 contract.

## (2) THE ROUND'S REAL OUTPUT, AND IT IS A CORRECTION TO R489

**R489's headline gold finding was a 101-day artifact and it is gone.**

R489 published `stop/vol` for PAXGUSD at **13.35 — "roughly FOUR TIMES the
constant"** — on 871 entries over 101 days, and called it "the one instrument
here that says the ratio is a property of the tape and not of the method" and
**"direct vindication of the standing transfer rule."** On 8,462 entries over
756 days the same code on the same instrument returns **3.79**, sitting
squarely inside the crypto band.

    instrument   entries   days  stopmed%  vol%med  stop/vol
    DOTUSD         15251    860    0.3044   0.0839      3.54   (was 3.59 on 118d)
    LINKUSD        28482   1627    0.3686   0.1037      3.57
    AVAXUSD        22396   1369    0.3630   0.0977      3.72   NEW
    LTCUSD         27805   1627    0.3418   0.0920      3.76
    ADAUSD          2190    132    0.3168   0.0799      3.78
    PAXGUSD         8462    756    0.3064   0.0834      3.79   (was 13.35 on 101d)
    BTCUSD         26771   1627    0.2054   0.0545      3.83   (R488 control)
    DOGEUSD        23471   1618    0.3943   0.0981      4.07   NEW
    XRPUSD         10389    750    0.3987   0.0841      4.50

**Nine crypto instruments, 165,000 entries, and every one of them sits between
3.54 and 4.50.** R488's 3.60 and 3.67 are inside that band. **There is now NO
...
What that does and does not mean, stated carefully because the temptation runs
the other way:
- **The transfer RULE is unchanged and is not weakened.** "Re-derive, never
  port" is right whether or not the ratio has ever been caught moving; it is
  the discipline that CAUGHT this, since porting R489's 13.35 forward is
  exactly what the rule forbade.
- **But its one piece of direct evidence is withdrawn.** Any future round that
  cites "gold sits at four times the constant" is citing a number computed on
  101 days that does not survive contact with 756. Nothing in this log
  currently demonstrates the ratio moving off crypto-like tape, because
  PAXG's tape IS crypto-venue tape and the two real equity-gold instruments
  (GLD, IAU) have never had a stop/vol reading taken at all.
- **The honest replacement claim is narrower and still worth having:** the
  ratio is stable at 3.5-4.5 across nine crypto instruments spanning 132 to
  1,627 days, and **a 101-day population is not enough tape to read it** — the
  two instruments that moved most between R489 and here are the two with the
  fewest days.

**PART 3 exists because of this and it needed its own fence.** step489's
part (b) iterates a hard-coded seven-name list written when AVAX and DOGE held
three days of tape, so the unchanged screen ranks two instruments it cannot
give a ratio for. Editing that list would have broken fence 4. Instead
step489 was **imported as a module** and its own `structural_stops`,
`daily_vol` and `cut80` called unmodified on AVAX and DOGE, with **PAXGUSD and
DOTUSD carried as controls** — both reproduce the screen's own table to four
decimals (8,462 / 756 / 0.3064 / 0.0834 / 3.79 and 15,251 / 860 / 0.3044 /
0.0839 / 3.54), which is what licenses the two rows that are not in it.

## (3) A STALE SENTENCE IN THE OUTPUT, PUBLISHED RATHER THAN EDITED AWAY

`step494_output.txt` contains the line **"GOLD (PAXGUSD) sits at 3.79 —
roughly FOUR TIMES the constant."** That is step489 printing a hard-coded
SENTENCE next to a COMPUTED number: the number was recomputed on the new tape
and moved, the prose was written when it read 13.35 and did not. The screen
was run unchanged per fence 4, so the contradiction is in the captured output
and is flagged here rather than quietly fixed. **Nobody may quote that
sentence.** It is a live demonstration that a conclusion baked into a print
statement outlives the measurement it describes — worth one line in this log
because this desk writes a lot of print statements.

## Honest limits

- **"As far back as the source will serve" is a statement about ALPACA, not
  about the asset.** ADA traded long before 2026-02-13; this vendor has no
  record of it. AVAX's 2021-11-18 start is likewise a listing date at this
  vendor, not the start of the market.
- **1-minute coverage runs 10.6% to 81.3%** on the backfilled files. Gaps are
  real and are NOT filled — R481 established this tape has holes. The screen's
  volatility is gap-clean so a hole is never counted as a move, but a
  hole-riddled tape is a thinner measurement than a full one at the same day
  count, and PAXG at 10.6% is the extreme case.
- **The spread half of every all-in cost is a fresh live seven-poll median.**
  The screen was run twice today; the **fee-only order is identical across
  both runs** (it is sample-free), and the only movement anywhere is DOGE and
  PAXG swapping places in the sensitivity read. Any difference between an
  R489 all-in number and an R494 one is a SAMPLE difference and a PRICE
  difference, not a finding.
- **This round ranks; it does not qualify.** No threshold is applied, no
  instrument is selected, and "DOGE arrives third" is a statement about where
  a look would be worth spending, not a proposal to spend one.

## Looks consumed

**NONE, and none could be.** No entry population was scored. `simulate()` was
never called, in this file or in the screen it ran. No return, expectancy, win
rate or risk multiple was computed for any instrument. Every tape measurement
stops at that instrument's own 80% boundary, so **XRPUSD's final 20% and every
other INTACT slice remain unread and unspent** — including the four slices
that only came into existence this round. No spent slice was touched or
argued to be reset. No order was placed, no account was created, no live file
was touched, imported or edited.

## What this opens

Three follow-ups, queued as items 24, 25 and 26. **Two of the four instruments
this round put on the board are worth a decision memo alongside XRP** — DOGE
brings 1,627 clean days and PAXG brings gold a coordinate seven times better
than the one R493 handed the specialist six days ago, and item 21 was written
...
# R498 — THE FIVE INDEX PERPETUALS ARE OPEN, PRICED, AND THE CHEAPEST FEE ON THE VENUE. THE SPREAD IS WHAT MAKES THEM EXPENSIVE.

**2026-09-11. Queue item 22. `step498_index_perp_session_poll.py`, full output
in `step498_out.txt`. Research only, no orders, no account, no live file
touched, imported or edited. NO LOOK CONSUMED, and none could be: `simulate`
is never called, no entry is built, no sweep is scanned, no return,
expectancy, win rate, risk multiple or t-statistic is computed for any
instrument. Every on-disk tape measurement stops at that instrument's own 80%
boundary. No sealed slice was loaded into any frame.**

## What this round was for

R493 tried to price US 500 PERP and TECH PERP and could not: all five
index-calendar perpetuals (US 500, TECH, AI, CHINA, DFNSE) were CLOSED for
the whole poll with $0 of 24-hour volume, and R493 refused to publish a frozen
weekend quote as a cost. Their 0.0400% fee floor was sourced and stood; the
spread half did not exist. Item 22 asked for the same sampler run inside the
contracts' own session, plus depth and volume, plus an R488 coordinate for
US 500 and TECH paired with SPY/QQQ tape and labelled a proxy.

## The session gate — R493's blocker is gone

Polled 2026-09-11 07:37 UTC, a Friday inside the US futures session. **All
five are OPEN with real two-sided volume**, and every one is far above the
$750 break point, so every one pays the venue's flat floor.

| contract | session | 24h $ volume | 24h contracts | notional $ | fee RT% |
|---|---|---|---|---|---|
| US 500 PERP | OPEN | 14,534,537 | 4,737 | 3,068.30 | 0.0400 |
| TECH PERP | OPEN | 36,409,442 | 9,277 | 3,924.70 | 0.0400 |
| AI PERP | OPEN | 5,773,302 | 1,986 | 2,907.00 | 0.0400 |
| CHINA PERP | OPEN | 2,599,168 | 923 | 2,816.00 | 0.0400 |
| DFNSE PERP | OPEN | 1,829,150 | 686 | 2,666.40 | 0.0400 |
| *BTC PERP (context)* | *OPEN* | *352,393,676* | *456,114* | *772.60* | *0.0400* |
| *ETH PERP (context)* | *OPEN* | *87,484,677* | *354,476* | *246.80* | *0.1216* |

**R493's $0-volume reading was a calendar artifact, not a property of the
contracts.** These are not dormant listings. TECH PERP turns over more dollars
in a day than XRP PERP did on R489's poll.

## (1) THE COST SHEET — seven polls, twelve seconds apart

R493's cadence unchanged, median carried with lo–hi so the noise stays
visible. Book age 0–2s on every contract; the gate never had to fire.

| contract | spread% med | lo | hi | fee RT% | **ALL-IN RT%** |
|---|---|---|---|---|---|
| **TECH PERP** | 0.0408 | 0.0382 | 0.0459 | 0.0400 | **0.0808** |
| **US 500 PERP** | 0.0424 | 0.0228 | 0.0424 | 0.0400 | **0.0824** |
| AI PERP | 0.0618 | 0.0550 | 0.0790 | 0.0400 | 0.1018 |
| CHINA PERP | 0.0853 | 0.0782 | 0.1030 | 0.0400 | 0.1253 |
| DFNSE PERP | 0.0937 | 0.0937 | 0.1012 | 0.0400 | 0.1337 |
| *BTC PERP* | *0.0130* | *0.0129* | *0.0130* | *0.0400* | *0.0530* |
| *ETH PERP* | *0.0203* | *0.0203* | *0.0203* | *0.1216* | *0.1418* |

all-in = fee round trip + one full top-of-book spread (R489/R493's formula,
unchanged).

**THE SHAPE OF THE ANSWER, and it inverts the expectation item 22 carried in
with.** R493(b) handed these to the index specialist as *"the cheapest fee on
the venue"*, and that is still true — 0.0400% is the floor and nothing on CDE
beats it. But **the fee is now the small half of every one of these five.**
The spread is 1.0x the fee on TECH and US 500 and **2.3x the fee on DFNSE.**
Against BTC PERP, where the spread is a third of the fee, these contracts are
the mirror image: the cheapest fee on the venue attached to the widest books
on it. The two contracts the desk already prices bracket them — BTC all-in
0.0530%, ETH 0.1418% — so the index five land squarely **inside the band the
desk already trades, not below it.**

## (2) DEPTH — and this is the number that actually binds

R480/R483 established that depth, not spread, caps account size. Contracts at
top of book converted to dollars at each contract's own notional; the figure
carried is the **worse side** (min of bid and ask), because a position has to
get out as well as in.

| contract | notional $ | top-of-book ct | **top-of-book $** | 10-level ct | 10-level $ | 24h $ vol |
|---|---|---|---|---|---|---|
| US 500 PERP | 3,068 | 1 | **3,068** | 194 | 595,250 | 14,534,537 |
| TECH PERP | 3,925 | 1 | **3,925** | 75 | 294,352 | 36,409,442 |
| AI PERP | 2,907 | 8 | **23,256** | 72 | 209,304 | 5,773,302 |
| CHINA PERP | 2,816 | 1 | **2,816** | 70 | 197,120 | 2,599,168 |
| DFNSE PERP | 2,666 | 1 | **2,666** | 74 | 197,314 | 1,829,150 |
| *BTC PERP* | *773* | *2* | *1,545* | *9,250* | *7,146,550* | *352,393,676* |
| *ETH PERP* | *247* | *18* | *4,442* | *16,606* | *4,098,361* | *87,484,677* |

**Four of the five quote exactly ONE contract at the touch.** The quoted
spread in section (1) is the price of a $2,700–$3,900 trade and of nothing
larger. The 10-level column is the honest capacity read — $197k to $595k a
side — and it is **12x to 36x thinner than BTC PERP's $7.1M** on the same
ten levels, against a 24-hour volume only 4x to 190x smaller. That is not a
disqualification; it is the number that belongs in front of any sizing
conversation, and it is the reason the spread column above should be read as
a floor rather than as what a real order pays.

## (3) THE R488 COORDINATE — the proxy the item asked for, plus the real tape

### (3a) Both reproduction controls pass exactly

| proxy | 80% slice | days | this round | R489 published | |
|---|---|---|---|---|---|
| SPY | 2016-01-01 → 2024-06-13 | 2,157 | 0.0173 | 0.0173 | **REPRODUCES** |
| QQQ | 2016-01-01 → 2024-06-13 | 2,156 | 0.0237 | 0.0237 | **REPRODUCES** |

SPY and QQQ are SPENT (R474). Nothing is scored on them here and nothing
could be — a volatility number is a property of the price series, not a
reading of any outcome.

### (3b) The round's unasked-for output: these contracts have their own tape

The venue serves 1-minute candles for every contract on the same keyless
public endpoint. No round has ever read them. That is strictly better
evidence than a proxy and sits inside the identical fence, so **both are
published.**

| contract | bars | days | **coverage** | own vol% median |
|---|---|---|---|---|
| US 500 PERP | 1,227 | 5 | 12.2% | 0.0172 |
| TECH PERP | 1,417 | 5 | 14.1% | 0.0286 |
| AI PERP | 464 | 5 | 4.6% | 0.0545 |
| CHINA PERP | 519 | 5 | 5.2% | 0.0547 |
| DFNSE PERP | 392 | 5 | 3.9% | 0.0613 |
| *BTC PERP* | *7,952* | *8* | *78.9%* | *0.0326* |
| *ETH PERP* | *7,398* | *8* | *73.4%* | *0.0487* |

**The proxy item 22 asked for is VINDICATED on US 500 and roughly right on
TECH.** US 500 PERP's own median minute is **0.0172%** against SPY's
**0.0173%** — a 0.6% difference on an eight-year proxy versus a five-day
tape. TECH PERP reads **0.0286%** against QQQ's 0.0237%, 21% higher. The
index perps track their underlying's percentage moves, as a scaled index
contract should.

### (3c) The coordinate, both ways

| contract | all-in %RT | proxy vol% | **proxy mult** | own vol% | **own mult** | coverage |
|---|---|---|---|---|---|---|
| US 500 PERP | 0.0824 | 0.0173 | **0.21** | 0.0172 | **0.21** | 12.2% |
| TECH PERP | 0.0808 | 0.0237 | **0.29** | 0.0286 | **0.35** | 14.1% |
| AI PERP | 0.1018 | — | — | 0.0545 | **0.53** | 4.6% |
| CHINA PERP | 0.1253 | — | — | 0.0547 | **0.44** | 5.2% |
| DFNSE PERP | 0.1337 | — | — | 0.0613 | **0.46** | 3.9% |
| *BTC PERP* | *0.0530* | *—* | *—* | *0.0326* | *0.62* | *78.9%* |
| *ETH PERP* | *0.1418* | *—* | *—* | *0.0487* | *0.34* | *73.4%* |

For scale, from R489 on the same coordinate: XRP 1.28, LINK 0.99, SOL 1.22,
BTC 1.04, ETH 0.49, QQQ 0.59, SPY 0.43 — everything but XRP and LINK spent.

### (3d) THE CALIBRATION THAT STOPS THE "own mult" COLUMN BEING MISREAD

**The own-tape column is internally comparable and NOT comparable to R489's
ranking, and the two context rows are what prove it.** BTC PERP's own tape
reads a median minute of **0.0326%** where R489's BTCUSD reads **0.0546%**
(ratio 0.60); ETH PERP reads **0.0487%** against R489's **0.0687%** (ratio
0.71). Same definition, same gap-clean filter, different window: R489 pooled
2021–2025, this round read one week of September 2026. **R476 already
established that this decay is real and large** (the crypto minute fell from
2021's +0.2908% to the 2026 stub's +0.0387%), so the ~0.65x gap is the
known decay showing up again, not an artifact of the endpoint.

Consequence, stated so nobody quotes the wrong column: **US 500 PERP's 0.21
and SPY's R489 0.43 are not the same measurement disagreeing — they are a
2026 week and a 2016–2024 pool.** The honest cross-round statement is the
proxy column (0.21 and 0.29), which is computed on exactly R489's tape and
reproduces its inputs digit for digit. The own-tape column's job is the
*relative* read among the five, where all rows share a window.

### (3e) What the coordinate says

**Every one of the five is under 1.0 on both reads, and four of five are
under 0.55.** The typical minute on these contracts does not cover the
crossing cost — US 500's covers about a fifth of it. On the relative read the
order is **AI (0.53) > DFNSE (0.46) > CHINA (0.44) > TECH (0.35) > US 500
(0.21)**, which is the *inverse* of the volume and spread ranking: the three
thematic contracts are more volatile by enough to outrun their wider books,
and the two liquid broad-index contracts are the worst coordinates on the
venue's index shelf. **US 500 PERP's 0.21 is the second-worst R488
coordinate this desk has ever measured on a priced contract**, above only
DOT's 0.18 — and DOT earned its place with a pathological fee (0.35% on an
$86.50 contract), where US 500 pays the venue's floor and is dragged down by
its tape alone.

## The handoff to the index specialist

| contract | all-in RT% | = fee + spread | top of book, a side | 10-level, a side | 24h $ vol | own vol% / mult |
|---|---|---|---|---|---|---|
| US 500 PERP | **0.0824** | 0.0400 + 0.0424 | $3,068 | $595,250 | $14.5M | 0.0172 / 0.21 |
| TECH PERP | **0.0808** | 0.0400 + 0.0408 | $3,925 | $294,352 | $36.4M | 0.0286 / 0.35 |
| AI PERP | **0.1018** | 0.0400 + 0.0618 | $23,256 | $209,304 | $5.8M | 0.0545 / 0.53 |
| CHINA PERP | **0.1253** | 0.0400 + 0.0853 | $2,816 | $197,120 | $2.6M | 0.0547 / 0.44 |
| DFNSE PERP | **0.1337** | 0.0400 + 0.0937 | $2,666 | $197,314 | $1.8M | 0.0613 / 0.46 |

Quoted at 2026-09-11 07:37–07:40 UTC, inside session, book age 0–2s.

**The desk's index specialist now has, for the first time, a primary-sourced
US perpetual round trip on a broad index — and it is 0.0808–0.0824%, roughly
sixteen times cheaper than the 0.50% Alpaca taker figure every index round in
this log has charged.** R474's SPY/QQQ result sits in the deployment-review
item at the top of the queue with a sealed net of +0.0326% of price per trade
and the honest note that its stop's tight tail sits at the scale of the
spread. **This round does not re-open or re-interpret that verdict and may
not be cited as doing so** — R474's numbers were computed on equity execution,
not on this contract, and porting a cost into a spent result is a second look
by arithmetic. What it does supply is the sourced cost sheet any *future*
pre-registered index round would be quoted at, and the depth number that caps
it.

## Honest limits

- **This is a seven-poll snapshot, not R480's 24-hour clock.** R480 found the
  one-minute sampler quoted a wider book than the clock on 5 of 6 series, by
  up to 29%, so these spreads are more likely pessimistic than optimistic —
  but that calibration was measured on crypto contracts, and **the index five
  have a session, which crypto does not.** A contract that shuts every day has
  an intraday spread profile this round did not sample at all.
- **The 21:00–22:00 UTC hole (R480) is untested on these contracts.** R480
  found Coinbase's crypto books empty to 5–11% of normal depth in that hour.
  These five *reopen* at 21:00 UTC. Whether the same hole exists, is worse, or
  is absent on an index contract is unmeasured and matters more here, because
  the 10-level depth is already 12–36x thinner than BTC's.
- **Coverage is the caveat that decides how much the own-tape column is
  worth, and it is item 25's caveat arriving on new instruments.** 12.2% and
  14.1% on US 500 and TECH are in PAXG's territory (10.6%), and **4.6% / 5.2%
  / 3.9% on AI, CHINA and DFNSE is well below it.** The gap-clean filter means
  a sparse tape contributes only its consecutive-minute pairs, which are by
  construction the contract's *busiest* minutes — so the own-tape volatility is
  biased **high**, and the own-mult column is a ceiling, not a centre. On AI,
  CHINA and DFNSE, with fewer than 500 bars each, the number is indicative and
  is labelled as such rather than ranked confidently.
- **The fee is still the cheapest the account can be, never the likeliest.**
  The CFM volume-tier ladder above the 0.02% floor remains unsourced after
  five attempts (R482, R486, R489, R493, this round). Every all-in figure here
  is a best case on the fee half.
- **Five days of tape and one poll window.** Nothing here establishes that
  these spreads or this volume persist. R493's own experience is the argument:
  the same five contracts read $0 and CLOSED six days ago.
- **Costs decide nothing** (owner rule, 2026-07-25). No figure above declines
  a trade, gates a strategy or ranks an instrument for trading. This is a cost
  sheet and a capacity read, handed to a specialist.

## Looks consumed

**NONE, and none could be.** `simulate` was never called. No entry population
was built on any instrument. No outcome, return, expectancy, win rate or risk
multiple was computed for anything. SPY's and QQQ's on-disk tapes were cut at
their own 80% boundary before any measurement, and the only thing read off
them was the size of a one-minute move. **XRPUSD, DOGEUSD, AVAXUSD and
DOTUSD's sealed slices are intact and unread; LINK's and the crypto and index
windows stay exactly as spent as they were.** No order was placed, no account
exists, no live file was touched or imported, and **nothing is proposed for
deployment.**

## What this closes and what it opens

**Item 22 is CLOSED.** Both halves delivered: all five priced inside their own
session with a published all-in round trip, and depth and volume recorded on
all five. The proxy pairing item 22 specified was delivered *and independently
validated* against the contracts' own tape, which no round had read.

Two follow-ups, queued as **items 31 and 32**:

- **31 — the 21:00 UTC hour on a contract that reopens at 21:00 UTC.** R480
  found a documented daily depth hole on CDE crypto at 21:00–22:00 UTC and
  showed it caps account size by an order of magnitude. These five contracts
  *begin their session* in that hour and already quote one contract at the
  touch. Sample the reopen window and the hour after it on all five, same
  sampler, same gates, and say whether the index shelf has the hole, a worse
  version of it, or none. Reading a live endpoint only; no look, no candidate.
- **32 — the own-tape coordinate on a real window.** This round proved the
  candles endpoint serves any CDE contract's 1-minute tape and that the desk
  can read it keyless. Five days at 4–14% coverage is indicative. Pull 60–90
  days for the five index contracts plus BTC/ETH as calibration, report
  coverage per contract, and re-state (3c) on a window long enough that the
...
# R500 — THE GOLD COORDINATE WAS NEVER READABLE. NEITHER THE OLD ONE NOR THE CORRECTION.

**2026-09-13. Queue item 25. `step500_gold_coordinate_restated.py`, full
output in `step500_output.txt`; an earlier same-day run of the identical file,
kept for its second spread sample, in `step500_output_run1.txt`. A CORRECTION
to a published coordinate, not a hypothesis. Research only, no orders, no
account, no live file touched, imported or edited. NO LOOK CONSUMED, and none
could be: `simulate()` is never called, no entry population is scored, and no
return, expectancy, win rate, risk multiple or t-statistic is computed for any
instrument. Every tape read stops at that instrument's own 80% boundary.**

## What this round was for

Item 25, opened by R494. R493 handed the gold specialist the first sourced US
perpetual cost gold has ever had here — PAXG PERP, 0.0806% all-in — and killed
the obvious next thought with it: paired with PAXG's own 1-minute tape the
R488 multiple read **0.58, "gold's median minute does not clear its own round
trip."** That used 118 days. R494 backfilled the file to 2,033 days and the
same code returned **0.0791% instead of 0.0467%**, a fee-only multiple of 1.98
and second place among intact instruments. The item asked for the handoff to
be restated on the longer tape, for a same-day book re-poll, and — the clause
that turned out to matter — for a plain answer to **whether a tenth-full tape
can carry a coordinate at all.**

## The three reproduction controls, all exact

Computed off the pre-backfill bytes in `data_pre494/`, not retyped, using
step489's own `daily_vol` and `cut80` imported unmodified.

- **R493/R489's 0.0467%** recomputes to **0.0467%** — diff 0.0000 pp.
- **R494's 0.0791%** recomputes to **0.0791%** — diff 0.0000 pp.
- **R494's raw column 0.1103%** recomputes to **0.1103%** — diff 0.0000 pp.
- And in part (4), **PAXGUSD returns stop/vol 3.79 and BTCUSD 3.83**, R494's
  published pair to the digit, which is what licenses the two new rows.

## (1) THE BOOK, RE-POLLED TODAY

| contract | 24h $vol | size | mark | notional | break px | need | fee RT% | spread% | **all-in RT%** |
|---|---|---|---|---|---|---|---|---|---|
| PAXG PERP | $366,097 | 1.00 | 4,358.30 | $4,358.30 | $750.00 | 0.17x | 0.0400 | 0.0482 | **0.0882** |

Seven polls twelve seconds apart, median carried. **R493's cost figure stands
and this round does not disturb it.** The mark moved −1.7% in eight days, the
contract is still far above the $750 break so **the fee half is a constant**
(R499's rule: PAXG PERP is one of the two contracts that cannot get cheaper on
this venue at any price), and every bit of the movement is spread.

**The spread is noisier than one sample admits, and this round accidentally
measured that.** The identical file was run twice today, about ninety minutes
apart: **0.0665% and 0.0482%**, all-in 0.1065% and 0.0882%. Both are
seven-poll medians. That 38% swing inside one day is the size of the sampling
noise R480's 24-hour clock exists to remove, and it is why the multiples below
are quoted to two digits and not three.

## (2) THE RESTATEMENT THE ITEM ASKED FOR

| read | 1m move% | fee RT% | all-in% | mult fee-only | mult all-in |
|---|---|---|---|---|---|
| R493, as published (118d) | 0.0467 | 0.0400 | 0.0806 | 1.17 | **0.58** |
| R494 tape + R493 cost (850d) | 0.0791 | 0.0400 | 0.0806 | 1.98 | 0.98 |
| R500 tape + today's cost (850d) | 0.0791 | 0.0400 | 0.0882 | 1.98 | 0.90 |

Taken at face value that is the item's deliverable and it would end here. It
does not, because of part (3).

## (3) THE ITEM'S OWN QUESTION, ANSWERED BY MEASUREMENT: **NO.**

The question was whether a tenth-full tape can carry a coordinate. That is not
answered by opinion, so it was answered by experiment. Take four instruments
whose 1-minute tape IS dense, measure the gap-clean coordinate on the full
tape behind the 80% fence — that is the truth — then **thin them to PAXG's own
presence pattern** (keep only the day × minute-of-day cells PAXG itself has a
bar in) and re-measure with the identical code. A second thinning keeps the
same number of bars per day chosen **uniformly at random** (seed fixed before
any number existed), which is thin but not selective, so the two columns
separate thinness from selection.

| donor | days | cov% | FULL | PAXG-MASK | ×FULL | RANDOM | ×FULL |
|---|---|---|---|---|---|---|---|
| BTCUSD | 1627 | 89.0 | 0.0546 | 0.0674 | **1.23x** | 0.0599 | 1.10x |
| DOGEUSD | 1627 | 82.3 | 0.1007 | 0.1302 | **1.29x** | 0.1168 | 1.16x |
| LTCUSD | 1627 | 72.9 | 0.0923 | 0.1189 | **1.29x** | 0.1116 | 1.21x |
| AVAXUSD | 1370 | 82.0 | 0.0973 | 0.1300 | **1.34x** | 0.1113 | 1.14x |

**Four for four, in the same direction, tightly: 1.23x–1.34x, median 1.29x.**
Random thinning at the same density inflates 1.10x–1.21x, median 1.15x — so
roughly **half the bias is thinness itself and half is the activity-selective
shape of PAXG's gaps.** Gold on a crypto venue prints when it trades, and the
minutes that survive to be 60-seconds-adjacent are disproportionately the busy
ones. The gap-clean definition protects the coordinate from counting a HOLE as
a move; it does nothing about the coordinate being conditioned on activity.

## (3b) AND THEN THE PREMISE OF THE ITEM ITSELF DID NOT SURVIVE

Two facts nobody had checked, both free, both on disk.

**FIRST: the two coordinates share zero bars.** R493's window behind its own
fence is **2026-03-01 → 2026-06-26**. The backfilled file's 80% fence lands at
**2025-06-15**, so R494's window is **2021-01-01 → 2025-06-15** — it ends
before R493's begins. **Overlap: 0 bars.** The "correction" is not a longer
read of the same tape. It is a disjoint read of a different era, and the two
numbers were never a short version and a long version of one thing.

**SECOND: PAXG's 1-minute tape is not continuous, and the fence sits inside
the hole.**

    2021-01 -> 2023-06    897 days   230,990 bars   17.8% full   258/day   gap-clean 0.0791%   (behind the fence)
    HOLE   2023-06 -> 2026-02        31 months with no 1-minute bar at all
    2026-02 -> 2026-07    165 days    80,302 bars   33.9% full   487/day   gap-clean 0.0485%   (spans/after the fence)

So the 850 days behind the fence are **entirely 2021–mid-2023**, and R494's
"10.6% coverage" is a whole-file figure; **behind the fence, where the
coordinate is actually computed, it is 17.8% and 258 bars a day** — which is
also a correction to the caveat the queue item asked to be attached.

**And inside gold's own file the bias is visible directly.**

| year | days | bars | cov% | bars/day | gap-clean% | raw% |
|---|---|---|---|---|---|---|
| 2021 | 365 | 65,651 | 12.5 | 180 | 0.0889 | 0.1013 |
| 2022 | 365 | 31,337 | **6.0** | 86 | 0.0780 | 0.1242 |
| 2023 | 167 | 134,002 | **54.9** | 802 | **0.0383** | 0.0670 |

**Correlation between coverage and the coordinate across the three years:
−0.95.** The years with the least tape read the largest minute, which is the
same direction and roughly the same size as the donor experiment measured on
instruments whose truth is known. **R494's 0.0791% is a weighted average of
two years at 6–12% coverage; it is a statement about gold's ACTIVE minutes.**

## THE HEADLINE

> **The 0.58 was unreadable rather than wrong — and so is the 1.98 that
> replaced it.** Every PAXG coordinate this desk has published rests on tape
> between 6% and 37% full, in a regime measured here to read 1.23x–1.34x high.
> The item anticipated this outcome for R493's number and asked for it to be
> said plainly; the run says it about R494's correction too.

**The one PAXG figure on this disk that is not mostly an artifact of absent
tape is the densest block, 2023: 54.9% full, 802 bars a day, gap-clean
0.0383% of price on 167 days.** It is LOWER than both published coordinates.
Paired with today's all-in it gives a multiple of **0.43**.

## THE CORRECTED HANDOFF TO THE GOLD SPECIALIST

- **COST — unchanged and sourced.** PAXG PERP, **0.0882% all-in today**
  (0.0400% fee, 0.0482% spread), CFTC-regulated venue, crypto calendar. Fee
  half is a constant above the $750 break; only the spread moves, and it moved
  38% between two runs ninety minutes apart.
- **TAPE — three numbers, not one, because the honest answer is a range.**

| read | tape quality | 1m move% | mult all-in (today) |
|---|---|---|---|
| R493 as published | 118d, 37.3% full, 2026 | 0.0467 | 0.53 |
| R494's 850d correction | 850d, 17.8% full, 2021-23 | 0.0791 | 0.90 |
| **gold's densest block** | **167d, 54.9% full, 2023** | **0.0383** | **0.43** |
| donor-implied de-biasing of R494's | indicative only | 0.0613 | 0.69 |

- **Fee-only, which is sample-free and the only pure price fact: 1.98.**
  R499's ranking put PAXG second on that read and **this round does not
  disturb the ranking** — it disturbs the numerator's reliability, which
  affects every row in that table that rests on a sparse tape, and PAXG is the
  sparsest by a distance.
- **The single sentence the specialist should carry: on every read this disk
  supports, gold's median minute is between 0.43 and 0.90 of its own all-in
  round trip, and not one of those reads is taken on tape more than half
  full.** The fee half is cleared roughly twice over; the spread half is what
  gold cannot clear.

## (4) GLD AND IAU — THE FIRST `stop/vol` EITHER HAS EVER HAD

step489's own `structural_stops`, `daily_vol` and `cut80`, imported and called
unmodified, behind each instrument's own 80% boundary, with two controls that
reproduce exactly. **Neither instrument has ever had an entry population built
on it by any round in this log, so nothing is spent by reading a stop
distance on them.**

| instrument | entries | days | stopmed% | vol%med | stop/vol | p25 | p75 |
|---|---|---|---|---|---|---|---|
| **GLD** | 875 | **81** | 0.1318 | 0.0333 | **3.79** | 2.03 | 5.95 |
| **IAU** | 754 | **81** | 0.2028 | 0.0358 | **4.87** | 2.43 | 9.52 |
| PAXGUSD *(control)* | 8462 | 756 | 0.3064 | 0.0834 | 3.79 | 1.93 | 6.91 |
| BTCUSD *(control)* | 26771 | 1627 | 0.2054 | 0.0545 | 3.83 | 2.34 | 5.99 |

**GLD lands at 3.79, dead inside R494's nine-instrument crypto band
(3.54–4.50). IAU lands at 4.87, just outside it, above.**

**Read both under R494's own lesson and neither is a finding.** Each rests on
**81 days** — fewer than the 101 that produced R489's 13.35, the number that
evaporated on more tape and is the whole reason this item exists. The two
equity-gold instruments were the last candidates on this disk that could have
shown the ratio departing from 3.5–4.5 off crypto-like tape; **GLD says it
does not, IAU hints that it might, and 81 days cannot tell those two apart.**
The honest statement is that **the ratio has still never been caught moving,
and the two instruments that could have caught it arrived too short to try.**
Their 1-minute tape begins 2026-03-02 at this vendor; a longer read is a data
question, not a research one, and is queued.

## Honest limits

- **The de-biasing factor is INDICATIVE and is not a measurement of gold.**
  1.29x is what PAXG's presence mask does to four crypto instruments. Gold's
  own intraday activity profile is not those four, and the correction is
  printed so the reader can see which side of 1.0 the multiple lands on, not
  so anybody can quote 0.0613% as gold's coordinate.
- **The spread is a seven-poll snapshot, twice over, not R480's 24-hour
  clock** — and this round is the first in the log to publish two same-day
  samples of the same book, which is why the noise is quotable at all.
- **`data_pre494/` is a backup of the pre-backfill bytes, not an independent
  source.** Control 1 proves the code and the window reproduce; it does not
  independently verify Alpaca's 2026 tape.
- **GLD/IAU stop distances come from the UTC-stamped parquets via `prep()`
  while their volatility comes from the ET-stamped ones the screen's UNIVERSE
  names.** The two differ only in day labelling, which moves a per-day median
  by a rounding error; it is recorded because the files are not the same file.
- **Nothing here says the method works on gold, on any venue, at any cost.**
  No entry population was scored anywhere in this round.

## Looks consumed

**NONE, and none could be.** `simulate()` was never called. No return,
expectancy, win rate, risk multiple or t-statistic was computed for any
instrument. Every tape read stops at its own 80% boundary, so **XRPUSD's final
20% and every other INTACT slice remain unread and unspent**, and GLD and IAU
enter this log with their slices whole. No spent slice was touched or argued
to be reset. No candidate is proposed. No order was placed, no account was
created, no live file was touched, imported or edited.

## What this opens

Three, queued as items 33, 34 and 35. **The tape-density bias is not gold's
problem alone** — R489/R494/R499's whole ranking divides by a gap-clean
coordinate, and the donor experiment says that coordinate is inflated wherever
coverage is low, so the ranking needs a coverage column and a sensitivity read
before any future round spends a look off it. **PAXG's 31-month hole should be
